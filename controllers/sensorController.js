...
const check_status=(req,res)=>{
  const level = req.query.level;
  newOccupancy.findOne({level:level},(error,data)=>{
    if(error){
      res.status(404).send(formarError(401,"No data found in database."))
    }else{
...
const get_all_levels=(req,res)=>{
  newOccupancy.find({},function(error,data){
    if(error){
      res.status(401).send(formarError(401,"No data found in database."))
    }else{
      res.send(data)
    }
  })
